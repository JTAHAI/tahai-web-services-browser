...
async function saveWorkspaceSnapshot(): Promise<void> {
  ...
  const name = window.prompt('Workspace name', defaultName)?.trim();
  if (!name) return;
  const snapshots = readJsonArray<WorkspaceSnapshot>(workspaceStorageKey);
...
function renderCommandPalette(): void {
  ...
  commandPaletteActions = buildCommandPaletteActions().filter((action) => !query || `${action.title} ${action.detail} ${action.group} ${action.shortcut || ''}`.toLowerCase().includes(query));
  ...
  commandPaletteList.innerHTML = commandPaletteActions.length ? commandPaletteActions.map((action, index) =>
    '<button class="command-row' + (index === commandPaletteIndex ? ' active' : '') + '" type="button" data-command-index="' + index + '">' +
    '<span><strong>' + escapeHtml(action.title) + '</strong><small>' + escapeHtml(action.group) + ' · ' + escapeHtml(action.detail) + '</small></span>' +
