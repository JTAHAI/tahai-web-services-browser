...
export function visibleMissionPaneIds(layout: MissionLayoutType, activePaneId = 'pane-1'): string[] {
  ...
  if (layout === 'focus') return [activePaneId || 'pane-1'];
  return ['pane-1'];
}
...
export function syncMissionLayoutPanesForMission(mission: MissionState): void {
  ...
  for (const paneId of missionPaneIds) {
    const tab = mission.tabs.find((candidate) => candidate.paneId === paneId && !usedTabIds.has(candidate.tabId));
    if (!tab) continue;
    usedTabIds.add(tab.tabId);
    panes.push({ paneId, role: tab.role, tabId: tab.tabId });
  }
  mission.layout.panes = panes;
  if (!mission.layout.activePaneId || !missionPaneIds.includes(mission.layout.activePaneId as typeof missionPaneIds[number])) mission.layout.activePaneId = 'pane-1';
}
