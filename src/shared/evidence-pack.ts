...
export function buildMissionEvidencePack(mission: MissionState, options: MissionEvidencePackOptions = {}): MissionEvidencePack {
  ...
  const tabs = mission.tabs.map((tab) => ({ role: tab.role, title: tab.title, url: tab.url, pane: tab.paneId }));
  const evidence = includeEvidence ? mission.evidence.map((entry, index) => ({ ref: `EV-${String(index + 1).padStart(2, '0')}`, kind: entry.kind, title: entry.title, url: entry.url, note: entry.operatorNote })) : [];
...
`## Notes\n\n${bullet(mission.notes)}\n`;
  ...
  return {
    ...
    label,
    markdown,
    redactedMarkdown: scan.redacted,
