...
#if BUILDFLAG(IS_WIN)
    ,
                                  public chrome::mojom::TahaiGuardDocument
#endif
{
 public:
...
#if BUILDFLAG(IS_WIN)
  void BindTahaiGuardDocument(
      mojo::PendingAssociatedReceiver<chrome::mojom::TahaiGuardDocument>
          receiver) override;
  void SetSelectors(const std::vector<std::string>& selectors) override;
  mojo::AssociatedReceiver<chrome::mojom::TahaiGuardDocument> guard_document_{
      this};
  blink::WebString guard_stylesheet_;
#endif
