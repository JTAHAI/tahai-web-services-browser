...
#if BUILDFLAG(IS_WIN)
  guard_document_.reset();
  guard_stylesheet_ = blink::WebString();
#endif
...
#if BUILDFLAG(IS_WIN)
void ChromeRenderFrameObserver::BindTahaiGuardDocument(
    mojo::PendingAssociatedReceiver<chrome::mojom::TahaiGuardDocument>
        receiver) {
  SetSelectors({});
  guard_document_.reset();
  guard_document_.Bind(std::move(receiver));
}

void ChromeRenderFrameObserver::SetSelectors(
    const std::vector<std::string>& selectors) {
  auto document = render_frame()->GetWebFrame()->GetDocument();
  if (!guard_stylesheet_.IsEmpty()) {
    document.RemoveInsertedStyleSheet(guard_stylesheet_,
                                      blink::WebCssOrigin::kUser);
    guard_stylesheet_ = blink::WebString();
  }
  if (selectors.empty() || !GURL(document.Url()).SchemeIsHTTPOrHTTPS() ||
      !tahai::guard::AreSafeCosmeticSelectors(selectors)) {
    return;
  }
  std::string css;
  ...
  guard_stylesheet_ = document.InsertStyleSheet(
