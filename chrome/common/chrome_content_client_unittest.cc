...
TEST(ChromeContentClientTest, AdditionalSchemes) {
  EXPECT_TRUE(url::IsStandard(tahai::kTahaiScheme));
  const GURL tahai_url("tahai://modes/");
  EXPECT_FALSE(std::ranges::contains(url::GetSecureSchemes(),
                                     tahai::kTahaiScheme));
  EXPECT_FALSE(network::IsUrlPotentiallyTrustworthy(tahai_url));
