...
TEST_F(TabRestoreServiceImplWithMockClientTest,
  ...
  split_tabs::SplitTabVisualData grid_data;
  grid_data.set_tahai_grid_ratios(0.35, 0.65);
  ON_CALL(mock_live_tab_context, GetVisualDataForSplit(quad_id))
      .WillByDefault(Return(&grid_data));
  ...
  EXPECT_CALL(
      mock_live_tab_context,
      ReconstructSplit(testing::SizeIs(4), quad_id, testing::Eq(grid_data)))
      .Times(1);
