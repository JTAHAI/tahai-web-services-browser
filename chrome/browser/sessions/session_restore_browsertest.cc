...
IN_PROC_BROWSER_TEST_F(SessionRestoreTest,
  ...
  expected.set_tahai_grid_ratios(0.35, 0.65);
  const auto split_id = model->AddToNewTahaiWorkspace({0, 1, 2, 3}, expected);
  model->ActivateTabAt(2);
  BrowserWindowInterface* restored = QuitBrowserAndRestore(browser());
  auto* restored_model = restored->GetTabStripModel();
  ASSERT_EQ(4, restored_model->count());
  ASSERT_TRUE(restored_model->ContainsSplit(split_id));
  EXPECT_EQ(expected, *restored_model->GetSplitData(split_id)->visual_data());
  EXPECT_EQ(4u, restored_model->GetSplitData(split_id)->ListTabs().size());
  EXPECT_EQ(2, restored_model->active_index());
  EXPECT_EQ(GetUrl1(),
            restored_model->GetWebContentsAt(0)->GetLastCommittedURL());
  EXPECT_EQ(GetUrl2(),
            restored_model->GetWebContentsAt(2)->GetLastCommittedURL());
}
...
IN_PROC_BROWSER_TEST_F(SessionRestoreTest,
  ...
  model->AddToNewTahaiWorkspace(
      {0, 1}, split_tabs::SplitTabVisualData(
                  split_tabs::SplitTabLayout::kStacked, 0.75));
  ...
  ASSERT_TRUE(tahai::NamedWorkspaceStore(browser()->GetProfile())
                  .Add(*snapshot.workspace));
  base::RunLoop flush;
  browser()->GetProfile()->GetPrefs()->CommitPendingWrite(flush.QuitClosure());
  flush.Run();
}

IN_PROC_BROWSER_TEST_F(SessionRestoreTest,
                       TahaiNamedWorkspaceSurvivesProcessRestart) {
  auto saved = tahai::NamedWorkspaceStore(browser()->GetProfile()).Read();
  ASSERT_TRUE(saved);
  ...
  EXPECT_EQ(split_tabs::SplitTabLayout::kStacked, visual->split_layout());
  EXPECT_EQ(0.75, visual->split_ratio());
  auto* mode = tahai::WindowModeController::GetForBrowser(restored);
  ASSERT_TRUE(mode);
