...
// Keep TAHAI's external-extension channel separate from Google Chrome.  A
// machine can legitimately have Chrome-specific extensions registered here
// (for example, a security-suite companion), but they must never be imported
// into TAHAI just because both applications are Chromium-based.
const wchar_t kRegistryExtensions[] =
    L"Software\\TAHAIWebServices\\TAHAIWebServicesBrowser\\Extensions";
