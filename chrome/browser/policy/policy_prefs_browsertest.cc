...
#if BUILDFLAG(IS_WIN)
// Keep every TAHAI mapping in the normal *Tahai* release test selection. A
// separate command-line mapping filter can silently omit a misspelled policy.
class TahaiPolicyPrefsTest : public PolicyPrefsTest {};

IN_PROC_BROWSER_TEST_F(TahaiPolicyPrefsTest,
                       TahaiAllEnterprisePoliciesMapToManagedPreferences) {
  base::ScopedAllowBlockingForTesting allow_blocking;
  base::ScopedTempDir cases;
  ASSERT_TRUE(cases.CreateUniqueTempDir());
  for (const char* name :
       {"LocalOIEnabled", "LocalOIMissionIngestionEnabled",
        "LocalOIArtifactIngestionEnabled",
        "LocalOIDocumentationReferenceIngestionEnabled",
        "LocalOIOpsToolIngestionEnabled", "LocalOIHistoryMetadataEnabled",
        "LocalOIReportsEnabled", "LocalOIExportEnabled",
        "LocalOIMspPromotionEnabled", "LocalOILocalAIEnabled",
        "LocalOIRetentionDays", "TahaiNamedWorkspacesEnabled",
        "TahaiGuardConfiguration", "TahaiGuardCustomRules", "TahaiSkinsEnabled",
        "TahaiSkinInstallationsAllowed"}) {
    const std::string filename = std::string(name) + ".json";
    ASSERT_TRUE(base::CopyFile(GetTestCaseDir().AppendASCII(filename),
                               cases.GetPath().AppendASCII(filename)))
        << "Missing TAHAI policy mapping: " << name;
  }
  ...
  ASSERT_TRUE(profile);
  VerifyPolicyToPrefMappings(
      cases.GetPath(), local_state, profile->GetOriginalProfile()->GetPrefs(),
      /*signin_profile_prefs=*/nullptr, GetMockPolicyProvider());
}
