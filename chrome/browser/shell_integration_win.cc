...
bool IsRunningAsPackagedApp() {
  ...
  // A process without package identity returns APPMODEL_ERROR_NO_PACKAGE. With
  // package identity, Windows reports the required buffer size even though no
  // buffer is supplied, which avoids allocating or exposing the package name.
  return ::GetCurrentPackageFullName(&package_full_name_length, nullptr) ==
