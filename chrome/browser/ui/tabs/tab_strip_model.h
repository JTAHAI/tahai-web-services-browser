...
class TabStripModel {
 public:
  ...
  // Updates both TAHAI grid dividers atomically through the existing split
  // observer path. Private windows keep this state in memory only.
  void UpdateTahaiGridRatios(split_tabs::SplitTabId split_id,
                             double row_ratio,
                             double column_ratio,
                             bool is_intermediate = false);
  ...
  // Creates a TAHAI native workspace from two to four existing tabs. The
  // active tab must be one of `indices`. Every pane remains an independent
  // Chromium tab and WebContents; only native presentation changes.
  split_tabs::SplitTabId AddToNewTahaiWorkspace(
      std::vector<int> indices,
      split_tabs::SplitTabVisualData visual_data);

  // Convenience wrapper for TAHAI's signature 2 x 2 workspace.
  split_tabs::SplitTabId AddToNewTahaiQuad(std::vector<int> indices);
