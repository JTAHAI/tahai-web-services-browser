...
void TabStripModel::UpdateTahaiGridRatios(split_tabs::SplitTabId split_id,
                                          double row_ratio,
                                          double column_ratio,
                                          bool is_intermediate) {
  ReentrancyCheck reentrancy_check(&reentrancy_guard_);
  ...
  split_data->visual_data()->set_tahai_grid_ratios(row_ratio, column_ratio);
  if (old_visual_data == *split_data->visual_data()) {
    return;
  }
  NotifySplitTabVisualsChanged(
      split_id, old_visual_data, *split_data->visual_data(),
...
split_tabs::SplitTabId TabStripModel::AddToNewTahaiQuad(
  ...
  return AddToNewTahaiWorkspace(
      std::move(indices), split_tabs::SplitTabVisualData(
...
void TabStripModel::MoveTabToIndexImpl(
  ...
  TabStripSelectionChange selection(GetActiveTab(),
                                    selection_model().GetListSelectionModel());
  if (move_within_split) {
    const int first_index = initial_split_tabs.front().second;
    contents_data_->MoveTabWithinSplit(tab->GetSplit().value(),
