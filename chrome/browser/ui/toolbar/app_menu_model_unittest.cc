...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
TEST_F(AppMenuModelTest, TahaiCommandsFollowTheActiveMode) {
  auto* mode_service = tahai::ModeServiceFactory::GetForProfile(
      browser()->GetProfile());
  ASSERT_TRUE(mode_service->SetActiveMode("research"));

  AppMenuModel research_menu(this, browser());
  research_menu.Init();
  EXPECT_TRUE(
      research_menu.GetIndexOfCommandId(IDC_TAHAI_LAUNCHPAD).has_value());
  EXPECT_TRUE(research_menu.GetIndexOfCommandId(IDC_TAHAI_COMMAND_CENTER)
                  .has_value());
  EXPECT_FALSE(
      research_menu.GetIndexOfCommandId(IDC_TAHAI_SUPPORT).has_value());

  ASSERT_TRUE(mode_service->SetActiveMode("support"));
  AppMenuModel support_menu(this, browser());
  support_menu.Init();
  EXPECT_TRUE(
      support_menu.GetIndexOfCommandId(IDC_TAHAI_SUPPORT).has_value());
  EXPECT_TRUE(support_menu.GetIndexOfCommandId(IDC_TAHAI_POLICY).has_value());
  EXPECT_FALSE(
      support_menu.GetIndexOfCommandId(IDC_TAHAI_LAUNCHPAD).has_value());
}
