...
void AppMenuModel::Build() {
  ...
  if (!chrome::IsTahaiMultiView(browser_->GetBrowserForMigrationOnly())) {
    AddItemWithIcon(
        ...
        ui::ImageModel::FromVectorIcon(kSplitSceneLeftIcon, ui::kColorMenuIcon,
                                       kDefaultIconSize));
    AddItemWithIcon(
        IDC_TAHAI_DUAL_VIEW_STACKED,
        l10n_util::GetStringUTF16(IDS_TAHAI_OPEN_DUAL_VIEW_STACKED),
        ui::ImageModel::FromVectorIcon(kSplitSceneUpIcon, ui::kColorMenuIcon,
                                       kDefaultIconSize));
    AddItemWithIcon(
        IDC_TAHAI_TRI_VIEW_TWO_OVER_ONE,
        l10n_util::GetStringUTF16(IDS_TAHAI_OPEN_TRI_VIEW_TWO_OVER_ONE),
        ui::ImageModel::FromVectorIcon(kSplitSceneDownIcon, ui::kColorMenuIcon,
                                       kDefaultIconSize));
    AddItemWithIcon(
        IDC_TAHAI_TRI_VIEW_ONE_OVER_TWO,
        l10n_util::GetStringUTF16(IDS_TAHAI_OPEN_TRI_VIEW_ONE_OVER_TWO),
        ui::ImageModel::FromVectorIcon(kSplitSceneRightIcon, ui::kColorMenuIcon,
                                       kDefaultIconSize));
  ...
  if (browser_->GetType() == BrowserWindowInterface::TYPE_NORMAL) {
    AddSeparator(ui::NORMAL_SEPARATOR);
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
    // Keep the app menu in lock-step with the native mode toolbar. Commands
    // are browser-owned and remain available, but the exposed work surface is
    // deliberately limited to the selected mode instead of presenting every
    // TAHAI feature as an undifferentiated list.
    // Incognito deliberately has no independently persisted TAHAI mode. Its
    // app menu reflects the regular profile's default without creating or
    // writing an off-the-record ModeService instance.
    ...
    const std::string_view mode =
        window_mode_controller
            ? window_mode_controller->active_mode_id()
    ...
    if (window_mode_controller) {
      ...
      AddRadioItem(IDC_TAHAI_RAIL_ICONS,
                   l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_ICONS),
                   IDC_TAHAI_RAIL_ICONS);
      AddRadioItem(IDC_TAHAI_RAIL_EXPANDED,
                   l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_EXPANDED),
                   IDC_TAHAI_RAIL_ICONS);
      AddRadioItem(IDC_TAHAI_RAIL_HIDDEN,
                   l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_HIDDEN),
                   IDC_TAHAI_RAIL_ICONS);
    }
#else
    AddItemWithStringId(IDC_TAHAI_LAUNCHPAD, IDS_TAHAI_OPEN_LAUNCHPAD);
    AddItemWithStringId(IDC_TAHAI_MISSION_CONTROL,
                        IDS_TAHAI_OPEN_MISSION_CONTROL);
    AddItemWithStringId(IDC_TAHAI_LOCAL_OI, IDS_TAHAI_OPEN_LOCAL_OI);
    AddItemWithStringId(IDC_TAHAI_COMMAND_CENTER,
                        IDS_TAHAI_OPEN_COMMAND_CENTER);
    AddItemWithStringId(IDC_TAHAI_WORK_MODES, IDS_TAHAI_OPEN_WORK_MODES);
    AddItemWithStringId(IDC_TAHAI_PROFILES, IDS_TAHAI_OPEN_PROFILES);
    AddItemWithStringId(IDC_TAHAI_SUPPORT, IDS_TAHAI_OPEN_SUPPORT);
    AddItemWithStringId(IDC_TAHAI_POLICY, IDS_TAHAI_OPEN_POLICY);
#endif
  }
