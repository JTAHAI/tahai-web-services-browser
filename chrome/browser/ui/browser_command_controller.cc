...
void BrowserCommandController::HandleCommandWithDisposition(
  ...
  switch (id) {
    ...
    case IDC_TAHAI_QUAD_VIEW:
      OpenTahaiQuadView(browser_);
      break;
    case IDC_TAHAI_DUAL_VIEW:
      OpenTahaiDualView(browser_, TahaiDualViewLayout::kSideBySide);
      break;
    case IDC_TAHAI_DUAL_VIEW_STACKED:
      OpenTahaiDualView(browser_, TahaiDualViewLayout::kStacked);
      break;
    case IDC_TAHAI_TRI_VIEW_TWO_OVER_ONE:
      OpenTahaiTriView(browser_, TahaiTriViewLayout::kTwoOverOne);
      break;
    case IDC_TAHAI_TRI_VIEW_ONE_OVER_TWO:
      OpenTahaiTriView(browser_, TahaiTriViewLayout::kOneOverTwo);
      break;
    case IDC_TAHAI_QUAD_FOCUS:
      SetTahaiMultiViewFocusMode(browser_,
                                 !IsTahaiMultiViewFocusMode(browser_));
      break;
    case IDC_TAHAI_QUAD_EXIT:
      ExitTahaiMultiView(browser_);
      break;
    case IDC_TAHAI_LAUNCHPAD:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiNewTabURL));
      break;
    case IDC_TAHAI_NAMED_WORKSPACES:
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
      tahai::ShowNamedWorkspaceManager(browser_);
#endif
      break;
    case IDC_TAHAI_GUARD_PANEL:
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
      tahai::guard::ShowGuardPanel(browser_);
#endif
      break;
    case IDC_TAHAI_SKIN_MANAGER:
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
      tahai::skins::ShowSkinManager(browser_);
#endif
      break;
    case IDC_TAHAI_FINDER:
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
      tahai::ShowFinder(browser_);
#endif
      break;
    case IDC_TAHAI_MISSION_CONTROL:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiMissionURL));
      break;
    case IDC_TAHAI_LOCAL_OI:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiLocalOiURL));
      break;
    case IDC_TAHAI_COMMAND_CENTER:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiOpsToolsURL));
      break;
    case IDC_TAHAI_WORK_MODES:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiModesURL));
      break;
    case IDC_TAHAI_PROFILES:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiProfilesURL));
      break;
    case IDC_TAHAI_SUPPORT:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiSupportURL));
      break;
    case IDC_TAHAI_POLICY:
      OpenTahaiSurface(browser_, GURL(tahai::kTahaiPolicyURL));
      break;
    ...
    case IDC_TAHAI_RAIL_HIDDEN:
      ...
      if (auto* controller =
        ...
        controller->SetActiveConfigurationValue(
            "rail_state",
            tahai::WindowModeController::RailStateForCommandId(id));
      }
#endif
      break;
    case IDC_TAHAI_MAKE_MODE_DEFAULT:
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
    {
      tahai::WindowModeController* const controller =
          tahai::WindowModeController::GetForBrowser(browser_);
      if (controller) {
        controller->MakeActiveModeProfileDefault();
      }
    }
#endif
    break;
...
void BrowserCommandController::InitCommandState() {
  ...
  for (int command :
       {IDC_TAHAI_RAIL_ICONS, IDC_TAHAI_RAIL_EXPANDED, IDC_TAHAI_RAIL_HIDDEN}) {
    command_updater_->UpdateCommandEnabled(command, false);
  }
#endif
...
#else
  command_updater_->UpdateCommandEnabled(IDC_TAHAI_SKIN_MANAGER, false);
  command_updater_->UpdateCommandEnabled(IDC_TAHAI_FINDER, false);
  command_updater_->UpdateCommandEnabled(IDC_TAHAI_NAMED_WORKSPACES, false);
  command_updater_->UpdateCommandEnabled(IDC_TAHAI_GUARD_PANEL, false);
#endif
  command_updater_->UpdateCommandEnabled(
