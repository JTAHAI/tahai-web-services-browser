...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
// TAHAI uses one adaptive command surface rather than a permanent row of
// product buttons. The selected mode determines the two contextual entry
// points; layout remains one stable control in every mode.
enum class TahaiToolbarMenuKind { kPrimary, kSecondary, kModes, kLayout };

std::string_view TahaiModeIdForCommandId(int command_id) {
  switch (command_id) {
    case IDC_TAHAI_MODE_DAILY:
      return "daily";
    case IDC_TAHAI_MODE_CREATOR:
      return "creator";
    case IDC_TAHAI_MODE_BUILDER:
      return "builder";
    case IDC_TAHAI_MODE_OPERATOR:
      return "operator";
    case IDC_TAHAI_MODE_RESEARCH:
      return "research";
    case IDC_TAHAI_MODE_SUPPORT:
      return "support";
    default:
...
std::string_view ActiveTahaiModeId(Browser* browser) {
  ...
  if (tahai::WindowModeController* controller =
          tahai::WindowModeController::GetForBrowser(browser)) {
    return controller->active_mode_id();
  }
  return tahai::ModeServiceFactory::GetForProfile(browser->GetProfile())
      ->active_mode()
      .id;
}
...
class TahaiToolbarMenuModel final : public ui::SimpleMenuModel,
 ...
 public:
    ...
    switch (kind) {
      ...
      case TahaiToolbarMenuKind::kSecondary: {
        ...
        AddItem(IDC_TAHAI_SKIN_MANAGER,
                l10n_util::GetStringUTF16(IDS_TAHAI_SKINS_MANAGER));
        break;
      }
      case TahaiToolbarMenuKind::kModes:
        AddRadioItem(IDC_TAHAI_MODE_DAILY, u"Daily Driver", 1);
        AddRadioItem(IDC_TAHAI_MODE_CREATOR, u"Creator Studio", 1);
        AddRadioItem(IDC_TAHAI_MODE_BUILDER, u"Builder Mode", 1);
        AddRadioItem(IDC_TAHAI_MODE_OPERATOR, u"Operator Mode", 1);
        AddRadioItem(IDC_TAHAI_MODE_RESEARCH, u"Research Desk", 1);
        AddRadioItem(IDC_TAHAI_MODE_SUPPORT, u"Support Desk", 1);
        AddSeparator(ui::NORMAL_SEPARATOR);
        AddItem(IDC_TAHAI_MAKE_MODE_DEFAULT, u"Use this mode for new windows");
        AddItem(IDC_TAHAI_WORK_MODES, u"Mode settings");
        AddSeparator(ui::NORMAL_SEPARATOR);
        AddRadioItem(IDC_TAHAI_RAIL_ICONS,
                     l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_ICONS), 2);
        AddRadioItem(IDC_TAHAI_RAIL_EXPANDED,
                     l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_EXPANDED), 2);
        AddRadioItem(IDC_TAHAI_RAIL_HIDDEN,
                     l10n_util::GetStringUTF16(IDS_TAHAI_RAIL_HIDDEN), 2);
        break;
      case TahaiToolbarMenuKind::kLayout:
        ...
        AddSeparator(ui::NORMAL_SEPARATOR);
        AddItem(IDC_TAHAI_QUAD_FOCUS,
                chrome::IsTahaiMultiViewFocusMode(browser_)
                    ? u"Show all panes"
                    : u"Focus active pane");
  ...
  bool IsCommandIdChecked(int command_id) const override {
    ...
    return !mode_id.empty() && mode_id == ActiveTahaiModeId(browser_);
  }
  ...
  void ExecuteCommand(int command_id, int event_flags) override {
    ...
    if (!mode_id.empty()) {
      if (browser_ && !browser_->GetProfile()->IsOffTheRecord()) {
        if (tahai::WindowModeController* controller =
                tahai::WindowModeController::GetForBrowser(browser_)) {
          controller->SetActiveMode(mode_id);
        }
      }
      return;
    }
...
class TahaiToolbarMenuButton final : public ToolbarButton {
 public:
  TahaiToolbarMenuButton(
      ...
      : ToolbarButton(views::Button::PressedCallback(),
                      ...
                      false),
        browser_(browser),
        menu_kind_callback_(std::move(menu_kind_callback)) {
 ...
 private:
  void ButtonPressed(const ui::Event& event) {
    active_menu_model_ = std::make_unique<TahaiToolbarMenuModel>(
        browser_, menu_kind_callback_.Run());
...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
void ToolbarView::OnTahaiWindowModeChanged() {
  UpdateTahaiModeControls();
}

void ToolbarView::UpdateTahaiModeControls() {
  if (!tahai_window_mode_controller_ || !tahai_mode_button_ ||
      !tahai_primary_button_ || !tahai_secondary_button_) {
    return;
  }
  ...
  if (mode.id == "daily") {
    ...
    primary_label = u"Workspace";
    secondary_label = u"Command Center";
    primary_icon = &kEditIcon;
    secondary_icon = &kRocketLaunchIcon;
  } else if (mode.id == "builder") {
    primary_label = u"Workspace";
    secondary_label = u"Command Center";
    primary_icon = &kCodeIcon;
    secondary_icon = &kTaskSparkIcon;
  } else if (mode.id == "operator") {
    primary_label = u"Mission";
    secondary_label = u"Support";
    primary_icon = &kSpeedIcon;
    secondary_icon = &kTableChartIcon;
  } else if (mode.id == "research") {
    primary_label = u"Workspace";
    secondary_label = u"Profiles";
    primary_icon = &kTextAnalysisIcon;
    secondary_icon = &kMenuBookIcon;
  } else {
    primary_label = u"Support";
    secondary_label = u"Policy";
    primary_icon = &kHelpCustomIcon;
    secondary_icon = &kTaskManagerOldIcon;
  }

  tahai_mode_button_->SetHighlight(mode_title, std::nullopt);
  tahai_mode_button_->GetViewAccessibility().SetName(
      u"Choose TAHAI work mode: " + mode_title);
  tahai_mode_button_->SetTooltipText(u"Mode: " + mode_title +
                                     u" (choose work mode)");
  tahai_primary_button_->SetHighlight(primary_label, std::nullopt);
  tahai_primary_button_->GetViewAccessibility().SetName(mode_title +
                                                        u" primary actions");
  tahai_primary_button_->SetTooltipText(mode_title + u" primary actions");
  tahai_primary_button_->SetVectorIcon(*primary_icon);
  tahai_secondary_button_->SetHighlight(secondary_label, std::nullopt);
  tahai_secondary_button_->GetViewAccessibility().SetName(
      mode_title + u" secondary actions");
  tahai_secondary_button_->SetTooltipText(mode_title + u" secondary actions");
  tahai_secondary_button_->SetVectorIcon(*secondary_icon);
  InvalidateLayout();
}

void ToolbarView::UpdateTahaiEnvironmentGuard(WebContents* tab) {
  if (!tahai_environment_button_) {
    return;
  }
  if (!tab) {
    tab = browser_->tab_strip_model()->GetActiveWebContents();
  }
  const std::optional<tahai::TahaiEnvironmentGuardRule> rule =
      tab ? tahai::FindTahaiEnvironmentGuardRule(
                browser_->GetProfile()->GetPrefs(), tab->GetVisibleURL())
          : std::nullopt;
  if (!rule || !rule->posture.show_persistent_boundary) {
    tahai_environment_button_->SetVisible(false);
    tahai_environment_button_->SetHighlight(std::u16string(), std::nullopt);
    InvalidateLayout();
    return;
  }

  const std::u16string label = base::UTF8ToUTF16(rule->posture.label);
  tahai_environment_button_->SetHighlight(label, std::nullopt);
  tahai_environment_button_->SetTooltipText(
      label + u" Environment Guard boundary. Open Local OI settings.");
  tahai_environment_button_->GetViewAccessibility().SetName(
      label + u" Environment Guard boundary");
  tahai_environment_button_->SetVisible(true);
  InvalidateLayout();
}
#endif  // BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
    ...
    if (!browser_->GetProfile()->IsOffTheRecord()) {
      std::unique_ptr<ToolbarButton> mode_button =
          std::make_unique<TahaiToolbarMenuButton>(
              browser_,
              base::BindRepeating([] { return TahaiToolbarMenuKind::kModes; }));
      mode_button->SetVectorIcon(kTableChartIcon);
      mode_button->GetViewAccessibility().SetName(u"Choose TAHAI work mode");
      tahai_mode_button_ = AddChildView(std::move(mode_button));

      std::unique_ptr<ToolbarButton> primary_button =
          std::make_unique<TahaiToolbarMenuButton>(
              browser_, base::BindRepeating(
                            [] { return TahaiToolbarMenuKind::kPrimary; }));
      primary_button->SetVectorIcon(kHomeIcon);
      primary_button->GetViewAccessibility().SetName(u"TAHAI primary actions");
      tahai_primary_button_ = AddChildView(std::move(primary_button));

      std::unique_ptr<ToolbarButton> secondary_button =
          std::make_unique<TahaiToolbarMenuButton>(
              browser_, base::BindRepeating(
                            [] { return TahaiToolbarMenuKind::kSecondary; }));
      secondary_button->SetVectorIcon(kTableChartIcon);
      secondary_button->GetViewAccessibility().SetName(
          u"TAHAI secondary actions");
      tahai_secondary_button_ = AddChildView(std::move(secondary_button));

      std::unique_ptr<ToolbarButton> environment_button =
          std::make_unique<ToolbarButton>(
              base::BindRepeating(callback, browser_, IDC_TAHAI_LOCAL_OI));
      environment_button->SetVectorIcon(kTaskManagerOldIcon);
      environment_button->SetVisible(false);
      environment_button->GetViewAccessibility().SetName(
          u"TAHAI Environment Guard boundary");
      tahai_environment_button_ = AddChildView(std::move(environment_button));

      tahai_window_mode_controller_ =
          browser_view_->tahai_window_mode_controller();
      if (tahai_window_mode_controller_) {
        tahai_window_mode_observation_.Observe(tahai_window_mode_controller_);
        UpdateTahaiModeControls();
      }
      UpdateTahaiEnvironmentGuard(
          browser_->tab_strip_model()->GetActiveWebContents());
    }

    std::unique_ptr<ToolbarButton> layout_button =
        std::make_unique<TahaiToolbarMenuButton>(
            browser_,
            base::BindRepeating([] { return TahaiToolbarMenuKind::kLayout; }));
    layout_button->SetVectorIcon(kGridViewIcon);
    layout_button->SetHighlight(u"Layout", std::nullopt);
    layout_button->GetViewAccessibility().SetName(u"Choose workspace layout");
    layout_button->SetTooltipText(u"Workspace layout (Dual, Tri, Quad, Focus)");
    tahai_layout_button_ = AddChildView(std::move(layout_button));
  }
#endif  // BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
  UpdateTahaiEnvironmentGuard(tab);
#endif
}
