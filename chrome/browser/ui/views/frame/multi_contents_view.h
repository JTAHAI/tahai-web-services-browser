...
// MultiContentsView shows native Chromium WebContents in single, two-pane,
// three-pane, and TAHAI Quad View layouts. Dual View supports side-by-side and
// stacked panes. Tri View supports 2-over-1 and 1-over-2 full-window rows.
// Chromium split tabs continue to own tab membership; this View owns only
// native presentation and focus routing.
class MultiContentsView
 ...
 public:
  ...
  // Returns the number of visible native pane containers.
  size_t GetVisibleContentsCount() const;
  ...
  // Reorders surviving panes without detaching their WebContents. Also handles
  // a closed member of a three- or four-pane split.
  void SynchronizeContentsInSplitView(
  ...
  // in [0, kMaxContentsViews). Assigning a secondary pane displays the native
  // multi-contents presentation.
  void SetWebContentsAtIndex(content::WebContents* web_contents, int index);

  // Clears every attached WebContents while preserving container allocation.
  // Used during atomic tab-collection swaps to avoid stale pane ownership.
  ...
  // Focus Pane temporarily presents only the active pane without destroying
  ...
  // Grid sizes are bounded even for tiny/zero viewports. All units are DIPs.
  static ViewSizes GetTahaiGridSizes(int extent, double ratio, int divider);
  double GetTahaiGridRatio(split_tabs::TahaiGridAxis axis) const;
  bool BeginTahaiGridResize(split_tabs::TahaiGridAxis axis);
  bool HasTahaiGridResizeTarget(split_tabs::TahaiGridAxis axis) const;
  void OnTahaiGridResize(split_tabs::TahaiGridAxis axis,
                         int resize_amount,
                         bool done_resizing);
  void ResetTahaiGridRatio(split_tabs::TahaiGridAxis axis);
  MultiContentsResizeArea* tahai_grid_resize_area_for_testing(
 ...
 private:
  void SwapContentsViews(size_t first, size_t second);
  ...
  raw_ptr<MultiContentsResizeArea> tahai_row_resize_area_ = nullptr;
  raw_ptr<MultiContentsResizeArea> tahai_column_resize_area_ = nullptr;

  struct TahaiGridResizeState {
    split_tabs::SplitTabId split_id;
    split_tabs::TahaiGridAxis axis;
    int start;
    int total;
  };
  ...
  // True while a Chromium split collection is attached, including Focus Pane
  // mode where only the active pane is intentionally visible.
  bool is_multi_contents_mode_ = false;
  bool tahai_focus_mode_ = false;
