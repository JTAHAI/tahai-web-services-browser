...
BrowserView::BrowserView(Browser* browser)
  ...
  if (tahai_window_mode_controller_) {
    tahai_workspace_rail_ =
        AddChildView(std::make_unique<tahai::WorkspaceRailView>(
            tahai_window_mode_controller_.get()));
  }
#endif
  ...
  switch (change.type) {
    ...
    case SplitTabChange::Type::kContentsChanged: {
      ...
      if (active_tab->GetSplit() == change.split_id) {
        ...
        int active_index = -1;
        for (tabs::TabInterface* tab : browser_->tab_strip_model()
                                           ->GetSplitData(change.split_id)
                                           ->ListTabs()) {
          if (tab == active_tab) {
            active_index = static_cast<int>(ordered_contents.size());
          }
          ordered_contents.push_back(tab->GetContents());
        }
        multi_contents_view_->SynchronizeContentsInSplitView(ordered_contents,
                                                             active_index);
      }
      break;
    }
...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(TAHAI_BRANDING)
  layout_views.tahai_workspace_rail = tahai_workspace_rail_;
#endif
