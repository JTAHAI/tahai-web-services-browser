...
struct BrowserViewTabbedLayoutImpl::HorizontalLayout {
  ...
  bool has_workspace_rail() const { return workspace_rail_width > 0; }
};
...
BrowserViewTabbedLayoutImpl::CalculateHorizontalLayout(
  ...
  // The workspace rail lives in the content area and never overlays web
  // contents. Allocate it independently from Chromium's side panel so both
  // may be visible in the same window.
  int min_workspace_rail_width = 0;
  int preferred_workspace_rail_width = 0;
  if (const auto* const rail = views().tahai_workspace_rail.get();
      IsParentedToAndVisible(rail, views().browser_view)) {
    min_workspace_rail_width = rail->GetMinimumSize().width();
    preferred_workspace_rail_width = std::max(
        min_workspace_rail_width, rail->GetPreferredSize().width());
  }
  ...
  layout.workspace_rail_width = min_workspace_rail_width;
  ...
  int remaining = params.visual_client_area.width() - layout.min_content_width -
                  ...
                  min_workspace_rail_width - min_side_panel_width;
  ...
  if (remaining > 0) {
    const int workspace_rail_flex_amount =
        std::min(remaining, preferred_workspace_rail_width -
                                min_workspace_rail_width);
    layout.workspace_rail_width += workspace_rail_flex_amount;
    remaining -= workspace_rail_flex_amount;
  }
...
BrowserViewTabbedLayoutImpl::CalculateProposedLayout(
  ...
  // Lay out the rail only after the top container and infobar have consumed
  // their vertical area. This keeps the normal tabstrip and omnibox geometry
  // unchanged while giving the rail a dedicated, non-overlapping content lane.
  if (horizontal_layout.has_workspace_rail() &&
