...
void BrowserWindowPropertyManager::UpdateWindowProperties() {
  // MSIX supplies the authoritative AUMI and tile icon for its full-trust
  // application. Do not overwrite that identity with Chromium's desktop
  // profile-specific AUMI, otherwise a Store-installed TAHAI window falls back
  // to the executable icon in Start and on the taskbar.
  if (shell_integration::win::IsRunningAsPackagedApp()) {
    return;
  }
