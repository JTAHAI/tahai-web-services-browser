...
check('matrix-pass-truth', matrix.pass === 'PASS345' && matrix.contractId === 'pass345-blackbox-electron-release-e2e-v1' && matrix.versionTarget === '2.0.18');
...
check('matrix-scenarios', Array.isArray(matrix.scenarios) && ['launch-shell-stage-webview', 'primary-browser-routing', 'chrome-flyouts-and-dialogs', 'tool-card-dialog-actions', 'browser-kit-find-and-guest-click', 'browser-history-session-recovery', 'browser-tab-pinning-and-switching', 'mission-control-layout-and-export'].every((id) => matrix.scenarios.some((entry) => entry.id === id)));
...
check('runner-covers-tool-card-dialog-actions', runner.includes("scenario.id === 'tool-card-dialog-actions'") && runner.includes('#capture') && runner.includes('#ops-check') && runner.includes('#it-card') && runner.includes('#endpoint') && runner.includes('[data-ops-action=\"command\"]') && runner.includes('[data-ops-action=\"shortcuts\"]'));
