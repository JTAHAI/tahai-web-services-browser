...
check('non-destructive-session-restore', (app.includes('if (existing.has(url)) continue;') || app.includes('if (!entry?.url || existing.has(entry.url)) continue;')) && app.includes("setStatus(createdTabIds.length ? 'Session restored' : 'Session already open'"), 'session restore appends missing tabs instead of destructively replacing the active workspace');
