...
const prev = 'PASS308';
const required = [
  'src/shared/pass309-crash-session-durability-contract.ts',
  'docs/qa/PASS309-crash-session-durability.md',
  'release-candidate/generated/pass309-crash-session-durability-report.json',
  'release-candidate/generated/pass309-crash-session-durability-manifest.json',
  'release-candidate/generated/pass308-privacy-local-data-inventory-report.json'
];
...
const prevReport = JSON.parse(fs.readFileSync(path.join(root, 'release-candidate/generated/pass308-privacy-local-data-inventory-report.json'), 'utf8'));
...
const contract = fs.readFileSync(path.join(root, 'src/shared/pass309-crash-session-durability-contract.ts'), 'utf8');
for (const token of [
  'PASS309_CRASH_SESSION_DURABILITY_PASS',
  'browser-side-only',
  'navLinkBrowser',
...
const report = JSON.parse(fs.readFileSync(path.join(root, 'release-candidate/generated/pass309-crash-session-durability-report.json'), 'utf8'));
...
for (const assertion of ["forcedKillRecoveryDocumented", "rendererCrashRecoveryDocumented", "failedWebviewRecoveryDocumented", "missionRestoreDocumented", "tabsRestoreDocumented", "layoutRestoreDocumented", "notesEvidencePreserved", "diagnosticShownInsteadOfSilentLoss", "browserSideOnlyScopePreserved", "storeSubmissionNotClaimed", "signedReleaseNotClaimed", "gaReleaseNotClaimed"]) {
...
if (report.nextPass !== 'PASS310 Enterprise Support Bundle v3') {
  console.error(`${pass} must hand off to PASS310 Enterprise Support Bundle v3.`);
  process.exit(1);
}

console.log('PASS309_CRASH_SESSION_DURABILITY=PASS');
