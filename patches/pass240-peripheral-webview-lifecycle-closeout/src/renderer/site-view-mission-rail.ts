(() => {
  ...
  // PASS169 delayed overlay focus guard: stale Site View focus timers must not refocus a hidden rail.
  ...
  let pass171SiteViewFocusEpoch = 0;
  let renderTimer = 0;
  let selectedIndex = 0;
  ...
  let favoriteSiteKeys = readFavoriteSiteKeys();
  ...
  function readFavoriteSiteKeys(): Set<string> {
    try {
      const parsed = JSON.parse(localStorage.getItem(RAIL_FAVORITES_KEY) || '[]');
      return new Set(Array.isArray(parsed) ? parsed.filter((value): value is string => typeof value === 'string').slice(0, 200) : []);
    } catch {
      return new Set();
    }
  }

  function writeFavoriteSiteKeys(): void {
    localStorage.setItem(RAIL_FAVORITES_KEY, JSON.stringify(Array.from(favoriteSiteKeys).slice(0, 200)));
  }

  function favoriteKeyForSnapshot(url: string, title: string): string {
    try {
      const parsed = new URL(url);
      return `${parsed.protocol}//${parsed.host}${parsed.pathname}`.toLowerCase();
    } catch {
      return (url || title || 'pending').trim().toLowerCase().slice(0, 220);
    }
  }
  ...
  function pass240IsWebviewApiReady(webview: CaptureCapableWebview | null): boolean {
    return Boolean(
      webview &&
  ...
  function getUrlFromWebview(webview: CaptureCapableWebview | null): string {
    if (!webview) return '';
    try {
      if (pass240IsWebviewApiReady(webview)) {
  ...
  function createToggleButton(): HTMLButtonElement {
    ...
    button.type = 'button';
    button.className = 'home-button secondary site-view-rail-toggle utility-chrome-button';
    ...
    button.innerHTML = '<span class="chrome-action-icon" aria-hidden="true">◫</span><span class="chrome-action-label">Site View</span>';
  ...
  function createRail(): HTMLElement {
    ...
    filterField.placeholder = 'Filter sites, hosts, docs, logs, pinned…';
    ...
    footer.textContent = 'Click thumbnail switches tab • Hover previews • Pin favorites • Ctrl+Alt+J/K next-prev • Ctrl+Alt+Shift+V refresh • C copy URL';
  ...
  function dispatchRailLayoutChange(reason: string): void {
  ...
  function pass116AnnounceSiteViewOpen(): void {
    ...
    document.body.dataset.pass122ActiveOverlayOpenedSource = 'site-view';
  ...
  function pass171BumpSiteViewFocusEpoch(reason: string): number {
    pass171SiteViewFocusEpoch += 1;
    document.body.dataset.pass171OverlayFocusEpochGuard = 'true';
    document.body.dataset.pass171SiteViewFocusEpoch = String(pass171SiteViewFocusEpoch);
    document.body.dataset.pass171SiteViewFocusEpochReason = reason;
    return pass171SiteViewFocusEpoch;
  }
  ...
  function pass117FocusFirstRailControl(rail: HTMLElement): void {
    ...
    const focusEpoch = pass171SiteViewFocusEpoch;
    window.setTimeout(() => {
      if (focusEpoch !== pass171SiteViewFocusEpoch) { document.body.dataset.pass171SiteViewFocusSkipped = 'stale-epoch'; return; }
      if (document.body.dataset.pass116ActiveOverlay !== 'site-view') { document.body.dataset.pass171SiteViewFocusSkipped = 'inactive-overlay'; return; }
      ...
      document.body.dataset.pass171SiteViewFocusAppliedEpoch = String(focusEpoch);
      target.focus();
    }, 80);
  }
  ...
  function pass170RestoreFocusToSiteViewOpener(target: HTMLElement | null): void {
    window.setTimeout(() => {
      document.body.dataset.pass170RestoreFocusTargetGuard = 'site-view';
      const activeOverlay = document.body.dataset.pass116ActiveOverlay || 'none';
      if (activeOverlay !== 'none' && activeOverlay !== 'site-view') {
        document.body.dataset.pass170RestoreFocusSkipped = `site-view:${activeOverlay}`;
        return;
      }
      if (!pass170ElementCanRestoreFocus(target)) {
        document.body.dataset.pass170RestoreFocusSkipped = 'site-view:invalid-target';
        return;
      }
      target.focus();
      document.body.dataset.pass170RestoreFocusApplied = 'site-view';
    }, 0);
  }
  ...
  function pass117SetRailFocusOpen(open: boolean, restoreFocus = false): void {
    ...
    if (restoreFocus && pass117SiteViewOpener) pass170RestoreFocusToSiteViewOpener(pass117SiteViewOpener);
  }
  ...
  function setRailOpen(open: boolean, persist: boolean, restoreFocus = false): void {
    if (open) {
      ...
      pass171BumpSiteViewFocusEpoch('open');
    }
    ...
    if (open) {
      ...
      pass171BumpSiteViewFocusEpoch('close');
  ...
  function collectSnapshots(): RailSnapshot[] {
    ...
    const webviewByTabId = new Map<string, CaptureCapableWebview>();
    webviews.forEach((webview) => {
      const tabId = webview.dataset.browserTabId || webview.dataset.pass106SiteViewTabId || '';
      if (tabId && !webviewByTabId.has(tabId)) webviewByTabId.set(tabId, webview);
    });
    const filter = filterInput?.value.trim().toLowerCase() || '';

    let snapshots = buttons.map((button, index) => {
      const browserTabId = button.dataset.browserTabId || button.dataset.pass106SiteViewTabId || '';
      const webview = (browserTabId ? webviewByTabId.get(browserTabId) : undefined) || webviews[index] || null;
      ...
      const favoriteKey = favoriteKeyForSnapshot(url, title);
      return {
        index,
        browserTabId,
        title,
        ...
        favorite: favoriteSiteKeys.has(favoriteKey),
        favoriteKey,
    ...
    if (filter) {
      snapshots = snapshots.filter((snapshot) => `${snapshot.title} ${snapshot.url} ${snapshot.host} ${snapshot.group} ${snapshot.favorite ? 'pinned favorite' : ''}`.toLowerCase().includes(filter));
    }
    snapshots = snapshots.sort((a, b) => Number(b.favorite) - Number(a.favorite) || a.index - b.index);
    return snapshots;
  }
  ...
  function refreshSnapshot(snapshot: RailSnapshot): void {
    activateSnapshot(snapshot);
    try {
      if (pass240IsWebviewApiReady(snapshot.webview) && typeof snapshot.webview?.reload === 'function') {
  ...
  function toggleFavorite(snapshot: RailSnapshot): void {
    if (favoriteSiteKeys.has(snapshot.favoriteKey)) {
      favoriteSiteKeys.delete(snapshot.favoriteKey);
      setShellStatus(`Unpinned ${snapshot.title}`, 'Site View favorites updated');
    } else {
      favoriteSiteKeys.add(snapshot.favoriteKey);
      setShellStatus(`Pinned ${snapshot.title}`, 'Favorite sites float to the top');
    }
    writeFavoriteSiteKeys();
    scheduleRender(0);
  }

  function sendToPane(snapshot: RailSnapshot, paneId: string): void {
    const browserTabId = snapshot.browserTabId || snapshot.button.dataset.browserTabId || '';
    if (!browserTabId) {
      setShellStatus('Mission pane send blocked', 'Site View could not resolve the source browser tab');
      return;
    }
    document.dispatchEvent(new CustomEvent('tahai-site-view-send-tab-to-pane', { detail: { browserTabId, paneId } }));
    selectedIndex = snapshot.index;
    window.setTimeout(() => {
      requestThumbnailCapture(snapshot.webview, true);
      scheduleRender(0);
  ...
  function renderCard(snapshot: RailSnapshot): HTMLElement {
    ...
    card.classList.toggle('favorite', snapshot.favorite);
    ...
    pageButton.title = `Switch to ${snapshot.title}`;
    pageButton.addEventListener('click', () => activateSnapshot(snapshot));
    pageButton.addEventListener('mouseenter', () => requestThumbnailCapture(snapshot.webview, false));
    ...
    chip.textContent = snapshot.favorite ? 'PINNED' : statusLabel(snapshot.status);
    ...
    actions.append(
      createActionButton(snapshot.favorite ? 'Unpin' : 'Pin', snapshot.favorite ? 'Remove from Site View favorites' : 'Pin this site as a favorite', () => toggleFavorite(snapshot)),
  ...
  function requestThumbnailCapture(webview: CaptureCapableWebview | null, force: boolean, allowWhenPaused = false): void {
    ...
    if (!pass240IsWebviewApiReady(webview)) return;
  ...
  function installPass164MoreToolsActionBridge(): void {
    document.addEventListener(PASS164_MORE_TOOLS_ACTION_EVENT, (event) => {
      if (!(event instanceof CustomEvent) || event.detail?.actionId !== 'site-view-rail-toggle') return;
      event.preventDefault();
      setRailOpen(true, true, true);
      document.body.dataset.pass164MoreToolsActionHandled = 'site-view-rail-toggle';
    });
  }
