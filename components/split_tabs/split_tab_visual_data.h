...
// Native TAHAI Tri/Quad dividers. These do not change tab membership or
// identity.
enum class TahaiGridAxis { kRows, kColumns };
...
class SplitTabVisualData {
 public:
  ...
  // Ratios are independent of Dual View's ratio and survive a layout switch.
  // Invalid persisted/input values fall back to balanced panes; finite values
  // are bounded so neither side can be collapsed by a corrupt session file.
  void set_tahai_grid_ratios(double row_ratio, double column_ratio);
  double tahai_row_ratio() const { return tahai_row_ratio_; }
  double tahai_column_ratio() const { return tahai_column_ratio_; }
 ...
 private:
  ...
  double tahai_row_ratio_ = 0.5;
  double tahai_column_ratio_ = 0.5;
};
