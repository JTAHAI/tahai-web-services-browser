...
void WriteTahaiSplitGridExtension(
    base::Pickle* pickle,
    const split_tabs::SplitTabVisualData& visual_data) {
  pickle->WriteUInt32(0x54414731);  // TAHAI grid format 1; append-only trailer.
  pickle->WriteDouble(visual_data.tahai_row_ratio());
  pickle->WriteDouble(visual_data.tahai_column_ratio());
}

bool ReadTahaiSplitGridExtension(base::PickleIterator* iterator,
                                 split_tabs::SplitTabVisualData* visual_data) {
  if (iterator->ReachedEnd()) {
    visual_data->set_tahai_grid_ratios(0.5, 0.5);
    return true;
  }
  uint32_t version = 0;
  double rows = 0.5;
  double columns = 0.5;
  if (!iterator->ReadUInt32(&version) || version != 0x54414731 ||
      !iterator->ReadDouble(&rows) || !iterator->ReadDouble(&columns) ||
      !iterator->ReachedEnd() || !std::isfinite(rows) ||
      !std::isfinite(columns) || rows < 0.1 || rows > 0.9 || columns < 0.1 ||
      columns > 0.9) {
    return false;
  }
  visual_data->set_tahai_grid_ratios(rows, columns);
  return true;
}
  ...
  for (const auto& command_ptr : data) {
    ...
    switch (command->id()) {
      ...
      case kCommandSetSplitTabData: {
        ...
        auto visual_data = split_tabs::SplitTabVisualData(
            SplitTabLayoutFromString(split_layout_str), split_ratio);
        if (!std::isfinite(split_ratio) || split_ratio < 0.0 ||
            split_ratio > 1.0 ||
            !ReadTahaiSplitGridExtension(&iter, &visual_data)) {
          return;
        }
        split->split_visual_data_ = visual_data;
        break;
      }
...
std::unique_ptr<SessionCommand> CreateSplitTabDataUpdateCommand(
  ...
  WriteTahaiSplitGridExtension(&pickle, *split_tab_visual_data);
