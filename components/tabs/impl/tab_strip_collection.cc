...
void TabStripCollection::MoveTabWithinSplit(split_tabs::SplitTabId split_id,
                                            size_t initial_index,
                                            size_t final_index) {
  SplitTabCollection* split = GetSplitTabCollection(split_id);
  CHECK(split);
  CHECK_LT(initial_index, split->TabCountRecursive());
  CHECK_LT(final_index, split->TabCountRecursive());
  if (initial_index == final_index) {
    return;
  }
  TabCollection::Position position = {
      split->GetHandle(),
      final_index + (initial_index < final_index ? 1u : 0u)};
  MoveTabImpl(split->GetTabAtIndexRecursive(initial_index), position);
}
