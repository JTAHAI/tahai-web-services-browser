...
class ZipReader {
 public:
  ...
  struct Entry {
    // Exact central-directory name bytes, before Unicode-extra-field handling
    // or path normalization. Security-sensitive consumers can reject aliases,
    // embedded NULs and metadata types even on Windows. These fields are
    // untrusted archive metadata, not guarantees about extracted contents.
    std::string raw_path;
    uint64_t compressed_size = 0;
    uint32_t external_attributes = 0;
    uint16_t compression_method = 0;
    bool has_unicode_path_override = false;
