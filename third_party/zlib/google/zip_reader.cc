...
bool ZipReader::OpenEntry() {
  ...
  if (info.size_filename >= path_in_zip.size()) {
    return false;
  }
  entry_.raw_path.assign(path_in_zip.data(), info.size_filename);
  entry_.compressed_size = info.compressed_size;
  entry_.external_attributes = info.external_fa;
  entry_.compression_method = info.compression_method;
  entry_.has_unicode_path_override = info.size_utf8_filename > 0;
